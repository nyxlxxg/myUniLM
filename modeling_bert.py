...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, is_hierarchical=False):
        ...
        if is_hierarchical:
            alpha = 0.4
            pos_ids = torch.arange(512, dtype=torch.long, device=input_ids.device)
            ...
            for i in range(position_ids.size(0)):
                pos_embedding_x = torch.index_select(position_embeddings, 0, position_ids[i,:] // 10)
                pos_embedding_y = torch.index_select(position_embeddings, 0, position_ids[i,:] % 10)
